CREATE PROCEDURE [dbo].[usp_Admin_HoldMember]
    @ClientId INT,
    @MemberId INT,
    @Reason NVARCHAR(500),
    @ChangedBy INT = NULL
...
BEGIN
    ...
    IF (@Reason IS NULL OR LEN(LTRIM(RTRIM(@Reason))) < 20)
    BEGIN
        RAISERROR('reason is mandatory (min 20 characters).', 16, 1);
        RETURN;
    END
    ...
    VALUES (@applicationId, @oldStatus, 'ON_HOLD', @ChangedBy, GETDATE(), @ChangedBy);
END
